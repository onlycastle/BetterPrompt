...
describe('KnowledgeGapWorker', () => {
  ...
  describe('execute()', () => {
    it('should return error when moduleAOutput missing', async () => {
      ...
      expect(result.data.overallKnowledgeScore).toBe(50);
    ...
    it('should return default neutral output on failure', async () => {
      ...
      expect(result.data.knowledgeGapsData).toBe('');
      expect(result.data.learningProgressData).toBe('');
      expect(result.data.recommendedResourcesData).toBe('');
      expect(result.data.topInsights).toEqual([]);
      expect(result.data.overallKnowledgeScore).toBe(50); // Neutral default
