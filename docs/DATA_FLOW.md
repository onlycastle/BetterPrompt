...
NoMoreAISlop은 Vercel의 4.5MB 페이로드 제한과 5분 타임아웃을 우회하기 위해 **하이브리드 아키텍처**를 사용합니다:
- **Vercel**: Next.js 웹앱, 프록시 역할
- **AWS Lambda (SST)**: 무거운 분석 API
- **CLI**: 사용자 로컬에서 세션 스캔 및 업로드
...
│   ├── X-Content-Encoding: gzip      (Vercel 인터셉트 방지)                   │
...
└───────────────────────────────┬─────────────────────────────────────────────┘
                                │
                                │ POST https://www.nomoreaislop.xyz/api/lambda/
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                         2. Vercel (Next.js)                                 │
...
│   │     destination: `${LAMBDA_API_URL}/:path*`,  ← 환경변수              │   │
│   │   }];                                                               │   │
│   │ }                                                                   │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│   환경변수:                                                                  │
│   └── LAMBDA_API_URL = https://xxx.lambda-url.ap-northeast-2.on.aws        │
│       (Lambda 배포 시 GitHub Actions가 자동 업데이트)                         │
│                                                                             │
│   역할: 프록시 (요청/응답 그대로 전달, Lambda URL 마스킹)                      │
└───────────────────────────────┬─────────────────────────────────────────────┘
                                │
                                │ Rewrite → Lambda Function URL
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                       3. AWS Lambda (SST v3)                                │
...
└───────────────────────────────┬─────────────────────────────────────────────┘
                                ...
                                ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                       4. CLI Response Handling                              │
│                                                                             │
│   packages/cli/src/uploader.ts:                                             │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │ SSE 스트림 처리:                                                     │   │
│   │ ├── progress 이벤트 → 터미널 진행률 표시                              │   │
│   │ ├── result 이벤트 → 최종 결과 반환                                    │   │
│   │ └── error 이벤트 → 에러 throw                                        │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
...
│   │                                                                     │   │
│   │ View full report: https://www.nomoreaislop.xyz/r/abc123             │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
└───────────────────────────────┬─────────────────────────────────────────────┘
                                │
                                │ Report URL
...
│   │    └── vercel env add LAMBDA_API_URL production --force             │   │
│   │        (Lambda URL 변경 시 자동 동기화)                               │   │
│   │                                                                     │   │
│   │ 6. Add Public Invoke Permission                                     │   │
...
CLI가 10MB 제한을 초과할 경우 자동으로 페이로드를 줄입니다:

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                     Payload Truncation (uploader.ts)                        │
│                                                                             │
│   Step 1: 전체 페이로드 시도                                                 │
│   ├── 모든 세션 포함                                                        │
│   ├── gzip 압축                                                             │
...
|-----------|------|---------|
| **CLI Entry** | `packages/cli/src/index.ts` | CLI 메인 진입점 |
| **Scanner** | `packages/cli/src/scanner.ts` | 세션 디렉토리 스캔 |
| **Uploader** | `packages/cli/src/uploader.ts` | 서버 통신, SSE 처리 |
| **Lambda Handler** | `lambda/analysis.ts` | Lambda 핸들러 (스트리밍) |
| **SST Config** | `sst.config.ts` | SST 앱 설정 |
| **Lambda Infra** | `infra/api.ts` | Lambda 함수 정의 |
| **Next Config** | `next.config.ts` | Vercel rewrite 설정 |
...
| Variable | Source | Purpose |
|----------|--------|---------|
| `LAMBDA_API_URL` | GitHub Actions (자동) | Lambda Function URL |
...
| Variable | Source | Purpose |
|----------|--------|---------|
| `NOSLOP_API_URL` | User (선택) | 커스텀 API URL (기본: nomoreaislop.xyz) |
| `GOOGLE_GEMINI_API_KEY` | User | Gemini API 키 (--api-key 플래그로도 가능) |
...
### Vercel Rewrite 미작동
```bash
# LAMBDA_API_URL 환경변수 확인
vercel env ls production

# 수동 업데이트
echo "https://xxx.lambda-url.ap-northeast-2.on.aws" | \
  vercel env add LAMBDA_API_URL production --force
```
...
# 2. Vercel 프록시 확인
curl -I https://www.nomoreaislop.xyz/api/lambda/

# 3. CLI 전체 흐름
cd packages/cli && npm run dev -- --api-key YOUR_KEY
```
