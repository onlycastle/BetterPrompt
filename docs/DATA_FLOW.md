...
> Version: 1.1.0 | Last Updated: 2025-01-17

## Overview

NoMoreAISlop은 Vercel의 4.5MB 페이로드 제한과 5분 타임아웃을 우회하기 위해 **직접 Lambda 호출 아키텍처**를 사용합니다:
- **Vercel**: Next.js 웹앱 (리포트 표시 전용)
- **AWS Lambda (SST)**: 분석 API (CLI가 직접 호출)
- **CLI**: 사용자 로컬에서 세션 스캔 → Lambda 직접 업로드
...
│                                                                             │
│   Note: CLI는 Lambda를 직접 호출하므로 Vercel rewrite 불필요                   │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
CLI가 25MB 제한을 초과할 경우 자동으로 페이로드를 줄입니다:
...
| Variable | Source | Purpose |
|----------|--------|---------|
| `NOSLOP_API_URL` | User (선택) | 커스텀 Lambda URL (기본: Lambda Function URL) |
