...
function buildUtteranceLookup(phase1Output) {
    return Object.fromEntries(phase1Output.developerUtterances.map((utterance) => [
        utterance.id,
...
export function buildPromptContext(input) {
    const { kind, phase1Output, deterministicScores, typeResult, domainResults, stageOutputs, domain, } = input;
    const base = {
        kind,
        availableDomains: domainResults.map((result) => result.domain),
        availableStages: Object.keys(stageOutputs).filter((key) => stageOutputs[key] !== undefined),
    };
    switch (kind) {
