...
import { JSONLLineSchema, CONTEXT_WINDOW_SIZE } from './types.js';
...
export function getScanCacheDir() {
 ...
 */
function deduplicateAssistantEntries(entries) {
...
export async function getSessionMetadata(filePath) {
    try {
        const content = await readFile(filePath, 'utf-8');
        const lines = content.split('\n').filter(l => l.trim());
        if (lines.length === 0)
        ...
        let avgContextUtilization;
        let maxContextUtilization;
