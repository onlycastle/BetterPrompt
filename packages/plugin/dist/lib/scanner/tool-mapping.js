 ...
 */
export function normalizeToolName(toolName, source) {
 ...
 */
export function getKnownTools(source) {
    return Object.keys(TOOL_MAPPING[source] || {});
}
/**
 * Check if a tool name needs normalization
 */
export function needsNormalization(toolName, source) {
 ...
 */
export function getToolCategory(canonicalToolName) {
    for (const [category, tools] of Object.entries(TOOL_CATEGORIES)) {
        if (tools.includes(canonicalToolName)) {
            return category;
        }
    }
    return 'OTHER';
 ...
 */
export const CURSOR_COMPOSER_TOOL_IDS = {
    15: 'run_terminal_cmd',
    38: 'edit_file',
    39: 'list_dir',
    40: 'write_file',
 ...
 */
export function resolveComposerToolId(numericId) {
