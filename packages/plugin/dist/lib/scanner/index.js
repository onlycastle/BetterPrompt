...
export class MultiSourceScanner {
    ...
    async parseSession(metadata) {
        ...
        try {
            ...
            return source.parseSessionContent(metadata.sessionId, metadata.projectPath, metadata.projectName, content);
        }
        catch {
            return null;
        }
 ...
 */
export async function hasAnySources() {
 ...
 */
export async function getAvailableSourceNames() {
    const sources = await sourceRegistry.getAvailable();
