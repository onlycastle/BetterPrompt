...
export class CursorComposerSource extends BaseSessionSource {
        ...
        try {
            ...
            for (const composerId of composerIds) {
                try {
                    ...
                    if (config?.minFileSize && estimatedSize < config.minFileSize)
                        continue;
                    if (config?.maxFileSize && estimatedSize > config.maxFileSize)
                        continue;
     ...
     */
    listComposerIds(db) {
        const stmt = db.prepare("SELECT key FROM cursorDiskKV WHERE key LIKE 'composerData:%'");
        const rows = stmt.all();
    ...
    countBubbles(db, composerId) {
        const stmt = db.prepare("SELECT COUNT(*) as count FROM cursorDiskKV WHERE key LIKE ?");
    ...
    getProjectDirFromBubbles(db, composerId) {
        const stmt = db.prepare("SELECT value FROM cursorDiskKV WHERE key LIKE ? LIMIT 20");
        const rows = stmt.all(`bubbleId:${composerId}:%`);
        for (const row of rows) {
            try {
                const bubble = JSON.parse(row.value);
...
//# sourceMappingURL=cursor-composer.js.map