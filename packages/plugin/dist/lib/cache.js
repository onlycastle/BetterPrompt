...
import { fetchUserSummary } from './api-client.js';
 ...
 */
export function setCachedSummary(userId, summary) {
    const database = getDb();
    database
        .prepare(`INSERT OR REPLACE INTO cached_insights
       (user_id, result_id, profile_json, growth_json, insights_json, fetched_at)
       VALUES (?, ?, ?, ?, ?, ?)`)
        .run(userId, summary.resultId, JSON.stringify({
        resultId: summary.resultId,
        analyzedAt: summary.analyzedAt,
        profile: summary.profile,
    }), JSON.stringify(summary.growthAreas), JSON.stringify({
 ...
 */
export function isCacheStale(userId) {
    const database = getDb();
    const row = database
 ...
 */
export async function getSummaryWithCache(userId) {
 ...
 */
export async function refreshCache(userId) {
    const fresh = await fetchUserSummary();
    if (fresh) {
        setCachedSummary(userId, fresh);
    }
