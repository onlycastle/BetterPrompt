...
async function run(): Promise<void> {
  const message = 'Background analyzer is removed. BetterPrompt now queues local Claude Code analysis for the next session; use `/analyze` and `sync_to_team` instead.';
  logError(message);
