...
// Runtime-only types (no compile-time imports from CLI package)
...
async function loadCliModules(): Promise<{
  scanSessions: (...args: unknown[]) => Promise<ScanResult>;
  uploadForAnalysis: (scanResult: ScanResult, token: string, ...args: unknown[]) => Promise<AnalysisResult>;
}> {
  // Try 1: installed npm package
  try {
    const [scannerMod, uploaderMod] = await Promise.all([
      import('betterprompt/scanner' as string),
      import('betterprompt/uploader' as string),
    ]);
    return {
  ...
  try {
    ...
    throw new Error(
      `Cannot import scanner/uploader. Install the betterprompt CLI package. ${importError}`,
    );
  }
}
