...
export function computeRunProgress(runId: number) {
...
export async function execute(_args: Record<string, unknown>): Promise<string> {
  ...
  return JSON.stringify({
    status: 'ok',
    ...progress,
    message: progress.completionStatus === 'incomplete'
      ? `Run #${runId} is incomplete (${progress.completedRequiredStages}/${progress.totalRequiredStages}${progress.skippedStageCount > 0 ? `, ${progress.skippedStageCount} skipped due to failures` : ''}). Resume with ${progress.nextStep.tool ?? progress.nextStep.skill ?? progress.nextStep.stage}.`
      : `Run #${runId} ${progress.skippedStageCount > 0 ? `completed with ${progress.skippedStageCount} skipped stage(s)` : 'already has all required stages'}. Ready to generate report.`,
  });
}
