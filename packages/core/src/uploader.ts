...
/**
 * Threshold for using S3 upload (5MB)
 */
...
async function uploadViaS3(
  ...
  onProgress?.('preparing', 10, 'Getting S3 upload URL...');
...
export async function uploadForAnalysis(
  ...
  const { accessToken, onProgress, lambdaUrl = DEFAULT_LAMBDA_URL } = options;
  ...
  if (compressedSizeBytes > USE_STORAGE_THRESHOLD) {
    onProgress?.('preparing', 0, `${sizeInfo} | Using S3 for large payload`);

    const { s3Key } = await uploadViaS3(compressed, lambdaUrl, accessToken, onProgress);
