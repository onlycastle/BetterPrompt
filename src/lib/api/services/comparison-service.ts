...
export interface FreeReportData {
  ...
  reportId: string;
  typeResult: TypeResult;
  // Free tier only gets basic type result
...
export const FEATURE_COMPARISON: FeatureComparisonItem[] = [
  ...
  },
  {
    ...
    description: 'How you compare to 10,000+ other developers',
    free: false,
    premium: true,
    category: 'advanced',
  },
  {
    feature: 'Session Trends',
    description: 'Track your progress over multiple sessions',
    free: false,
    premium: true,
    category: 'tracking',
  },
...
export function transformToFreeTier(report: ReportData): FreeReportData {
  const lockedFeatures = FEATURE_COMPARISON.filter((f) => !f.free).map((f) => f.feature);
...
export function getComparisonStats(): {
  ...
  const premiumOnlyCount = FEATURE_COMPARISON.filter((f) => f.premium && !f.free).length;
